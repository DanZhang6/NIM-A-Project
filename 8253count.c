...
float  Mean_Value[8];				//平均值
float  idata save0[8],save1[8],save2[8];//前5次DoseRata数据不要，save0,1,2用来记录前面3次的DoseRata(只平均三次的值)，来平滑计算平均值!



uchar  Flag_dw;                      //单位标志
uint  idata Tdata;                   //探头计数率
...
void ShowData()
{
	...
	float idata ptemp,jtemp,rata,yudecide,zdata,jishuguan_rata,jishuguan_jtemp,dianlishi_jtemp;
	//uint  mtemp;
...
{	
	...
	{    
	    Tdata=0;                                        //探头计数率uint,紫色为全局变量，蓝色为局部变量
		zdata=0;										//float
		rata=0;											//float
		jtemp=0;										//float
        ptemp=0;										//float
       ...
       if(j<7)											//(3)前七个探头既有电离室也有计数器，第八个只有电离室故分开处理
		{
			Var1=(Var_Signal1 & Svar1[j]);              //(4)探头控制信号1,var-signal探头控制标志澹(全接计数管)；Svar1[8]={0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80}
			if(Var1 == Svar1[j])
			{
			  
			  	Tdata = buf[4*j+4]*256+buf[4*j+3];		//buf为锁存的8253计数器高八位低八位，共16位
			  	//compare[i]=Tdata;
			...
			{
			 	 Tdata = buf[4*j+6]*256+buf[4*j+5];
			 	 //compare[i]=Tdata;
			 	 //if((compare[j]==compare[j+Incinput])&&(compare[j+Incinput]==compare[j+Incinput*2])&&(compare[j+Incinput*2]==compare[j]))
			     // Tdata=0;
			  		Var_Signal1 = Var_Signal1 & Svar0[j];	//Svar0[8]={0xFE,0xFD,0xFB,0xF7,0xEF,0xDF,0xBF,0x7F}
			}
			/*通过刷新时间标志Redrawtime和从8253采集来的计数值Tdata计算探测器计数率*/
			if(count_change_flag[j]==0)					//(4)计数改变标志，初始化count_change_flag均为0
			{
				...
				else if (Redrawtime < 60 )				   //修改2012.6.15 ，因为小于60秒与大于60s的计算方法不同
				{
                	...
                	rata =Tdata*zdata/10;				  //修改2012.7.1 
				}
			}
			else if(count_change_flag[j]==1)				//在Dose_Rata大于1000时Count_Change_flag置1
			{
			...
			if(DoseRata[j] < 0.15)	              // 修改2012.6.15 && (DoseRata[j] >= 0)
			{
			     ...
			     DoseRata[j] = (float)(Para[1]*(rata-Para[0]));
				 if(DoseRata[j]<0)
				 DoseRata[j]=0.0;
			...
			{    
			     ...
			     DoseRata[j] = (float)(Para[3]*(rata-Para[2])); 
				 if(DoseRata[j]<0)
				 DoseRata[j]=0.0;
			     Var_Signal1 = Var_Signal1 | Svar1[j];                   //控制信号1接计数管，为高电平
			     Var_Signal2 = Var_Signal2 & Svar0[j];                   //控制信号2接电离室10(8),低电平
		    ...
		    {  
			     ...
			     DoseRata[j] = (float)(Para[5]*(rata-Para[4]));
			     if(DoseRata[j]<0)
				       DoseRata[j]=0.0;  
            ...
            {   
                 ...
                 DoseRata[j] = (float)(Para[7]*(rata-Para[6]));
			     if(DoseRata[j]<0)
				       DoseRata[j]=0.0;  
			     Var_Signal1 = Var_Signal1 & Svar0[j];                   //控制信号1接计数管，低电平
		    ...
		    else if(DoseRata[j] >= 1000000)                              //当剂量率超过1Gy/h时，使用电离室的第一个欧姆档(10(6)),有待具体确定
		    {
			     ...
			     DoseRata[j] = (float)(Para[9]*(rata-Para[8]));
			     if(DoseRata[j]<0)
				       DoseRata[j]=0.0;  
			...
			if(Flag_Meant == 1) 
		  	{	
			     //save[isave]=DoseRata[j];
			   	if(jishucount==5)
				{	  
			        Mean_Value[j]=DoseRata[j];
					save0[j]=DoseRata[j];
				}
				else if(jishucount==6)
				{
					Mean_Value[j]=(float)((save0[j]+DoseRata[j])/2);
					save1[j]=DoseRata[j];
				}
				else if(jishucount==7)
				{   
				    Mean_Value[j]= (float)((save0[j]+save1[j]+DoseRata[j])/3);	    //平滑求均值   add 2012.6.15
					save2[j]=DoseRata[j]; 
				}
				else if(jishucount>7)							 
				{
					...
					save1[j]=save2[j];
					save2[j]=DoseRata[j];
					Mean_Value[j]=(float)((save0[j]+save1[j]+DoseRata[j])/3);					
				}	
		...
		else if(j == 7)                        //NIM_A的第八个探头只有高压电离室，且只有一个欧姆档
        {
			...
			if(Flag_Meant == 1) 
		  	{	
				...
				{
					Mean_Value[j]=(float)((Mean_Value[j]+DoseRata[j])/2);
					save1[j]=DoseRata[j];
				}
				else if(jishucount==7)
				{   
					save2[j]=DoseRata[j]; 
				    Mean_Value[j]= (float)((save0[j]+save1[j]+save2[j])/3);	    //平滑求均值   add 2012.6.15
				}
				else if(jishucount>7)							 
				{
					save0[j]=save1[j];
		...
		Signal3 = Var_Signal3;
		/**********单位换算后为jtemp************/
		if(Mean_Value[j] < 0)	                                      //修改2012.6.15 通过均值计算
		{
		    jtemp = 0.0;
		}
		else if(Mean_Value[j] >= 0)										//Mean_Value平均值单位为uGy/h
		{
			if(Mean_Value[j] < 1000)									//Mean_Value<1000表示单位为uGy/h
			{
			    jtemp = (float)Mean_Value[j];							
				Flag_dw = 1;
			}
			else if((Mean_Value[j] >= 1000)&&(Mean_Value[j] < 1000000))	//1000<Mean_Value<1000000表示mGy/h
			{
			    jtemp = (float)(Mean_Value[j]/1000);
				Flag_dw = 2;
			}
		    else if(Mean_Value[j] >= 1000000)							//Mean_Value>=1000000表示Gy/h
			{
			    jtemp = (float)(Mean_Value[j]/1000000);
				Flag_dw = 3;					
			}
		}
       /***********剂量率转换单位后转换成可以显示的BCD码*************/
        ...
        /***********判断阈值*************/
        mtemp=(YuThouth[j]+YuCent[j]*0.1+YuTenth[j]*0.01);	    //暂时没根据位数修改  2012.6.15
		...
		if(Tbcd[5] != 0)							//若百位非零，根据数的大小来显示
		{
			...
			if(Tdata<10)							//用于显示Tdata锁存的计数值显示
			  	Tnumber(576,13+(j*58),Tdata);	     //测试完成后去掉	2012.6.24
		...
		else
		{
			if(Tbcd[4] != 0)						//百位为零十位非零
			{
				...
				if(Tdata<10)
					...
					Tnumber(496,13+(j*58),Tdata);	 //测试完成后去掉	 2012.6.24
			 }
			 else									//百位十位位均为零，个位非零
			 {
			 	Tnumber(240,13+(j*58),Tbcd[3]);
       ...
       /**********计量单位显示***********/
		if(Flag_dw==1)					      //测试完成后加上  2012.6.24
		{   
		...
		//jishu++;
		//if(jishu == 65535)
		      //jishu=jishu%(Incinput*3);
		//count++;
		//if(count == 65535)
		      //count=count%(Incinput*2);
	}			//8个探头均显示执行完
	  //cj++;
	  Flag_Warn_Count=1;
